---
title: The hinge in the attention argument
description: If the machines do the doing, the answer on offer is that leadership becomes attention. Read Ashby and Lewin literally and attention becomes one more thing that has to be regulated, by somebody standing inside the same problem.
tags:
  - systems thinking
...
cover:
  ...
  alt: "A door hinge rendered as a technical illustration, but the central pin of the hinge is a human silhouette, standing upright, holding the two leaves together. The left leaf is dense and disordered: overlapping raw material, weather, cables, half-legible signals, textures that do not resolve. The right leaf is clean and ruled: a severity matrix grid, policy clauses, an architecture diagram with tidy boxes and arrows."
  relative: false
---

[Cassie Kozyrkov's question](https://purple.tymyrddin.dev/docs/thirteen/designing-what-machines-notice) is a good one:
if machines now do the doing, what is left for leadership?

Her answer is attention. Leadership becomes the design of what machines notice, optimise and repeat. That is a useful
shift, but the argument becomes more interesting if Ashby's law and Lewin's field theory are read a little more
literally than the familiar management versions allow.
...
B = ƒ(P, E) is not simply a reminder that people have characteristics. E is the lifespace, the situation as the person
actually reads it. What lies outside that field has no purchase on the behaviour, however real it may be from somewhere
else.

So an attention system cannot simply decide what the person will notice from outside the situation. It can surface
distinctions the person can recognise. It can make some things harder to miss. It can also institutionalise a blind
spot.

That changes what "attention scaffolding" can mean. The scaffold is not outside the control problem. Someone decides
what it surfaces, what it suppresses and what counts as a useful distinction. That decision is itself made from
...
That makes attention less like a scarce resource to be optimised and more like part of the control problem itself, and
whoever designs it is inside that problem too.
