...
An uncomfortable truth: Every byte uploaded to AWS, Azure, or Google Cloud isn't just data, it's *cloud capital*. Coined by economist Yanis Varoufakis, the term captures how tech giants transform digital activity into privatised infrastructure. It's not merely about hosting files; it's about hoarding power. And right now, the US holds the keys, turning Europe into a lodger in its digital manor.

But there is an alternative: cloud-on-prem, a return to digital self-sufficiency, and European providers like Hetzner, who offer control, compliance, and a way to starve the beast. Think of it less as going backwards and more as refusing to pay rent to a colonial landlord.

Here's why dumping Big Tech's cloud might be the most rebellious, and most reasonable, thing an organisation could do this year.
...
Imagine a medieval fiefdom, only instead of land, the tenant is tied to the whims of AWS billing and Google's usage quotas. A "freemium" tier is their fealty.

### Europe's weak hand

As of [Q2 2025](https://www.techopedia.com/cloud-market-share-leaders), the US controls approximately 63% of the 
global cloud infrastructure market, with AWS holding 30%, Microsoft Azure 20%, and Google Cloud 13%. Europe, for all 
its regulatory might, is playing catch-up with both hands tied behind its back. This isn't simply a business concern, 
...
* Data sovereignty: US clouds fall under the [CLOUD Act](https://www.congress.gov/bill/115th-congress/house-bill/4943/text), significantly weakening GDPR protections if backups live in Virginia. The legal grey area creates genuine compliance risks for European organisations.
...
Cloud-on-prem, or hybrid setups, give cloud-like features *without* the overlords. With platforms like OpenStack, Kubernetes, or even a good old Proxmox cluster, an organisation can self-host without self-harm.

Benefits include:

* No surprise invoices: Azure's infamous egress fees become someone else's horror story.
* Actual compliance: Store medical or legal data where it belongs, under the appropriate jurisdiction.
* Performance gains: Transatlantic latency is fine for email, less so for anything real-time.

That said, cloud-on-prem comes with its own challenges: it needs skilled staff to manage infrastructure, capital investment for hardware, and systems that can handle demand spikes. The hybrid approach acknowledges these realities: keep hyperscale where it genuinely makes sense, rather than letting it become the default.
...
Other European providers include OVHcloud (France), Scaleway (France), and IONOS 
...
Need ARM64 nodes that use 30% less energy than x86? Hetzner has them. A predictable bill at the end of the month? Also covered.
...
Quitting cold turkey isn't necessary. Bursty workloads (like AI training or render farms) can stay on AWS. But core data, personally identifiable information, legal documents, customer records, belongs on local kit or European providers. Hybrid is often less a compromise than a rebalancing of power.

This approach acknowledges the genuine technical advantages of hyperscale (global CDN networks, instant scaling, massive GPU clusters for AI) whilst reclaiming sovereignty over the most sensitive assets.

## Reclaiming cloud capital

The cloud was meant to free us from hardware, not hand us over to digital landlords. Three moves follow from adopting cloud-on-prem or hybrid models:

1. Starve technofeudalism: every workload moved is one less tithe to Silicon Valley.
2. Strengthen European tech: back providers that don't treat GDPR like an optional questline.
3. Future-proof compliance: Schrems II set the precedent, and Schrems III is surely in the mail.

As Varoufakis argues, cloud capital functions much like oil did in the colonial era, a strategic resource that defines power relationships between nations. In his analysis, Europe remains the colonised rather than the coloniser in this new digital economy. Time to sink a digital well of one's own.

## Starting a cloud exodus

1. Audit the workloads: identify lock-in, technical debt, and cost traps.
...
3. Go hybrid with intent: Keep hyperscale where genuinely needed (AI training, global CDN, extreme burst capacity), but don't let it become the default.
4. [Make a checklist like this one](https://blue.tymyrddin.dev/docs/dev/devsecops/cicd/exit-checklist).

The cloud isn't just infrastructure, it's a power structure, and one best chosen with both eyes open.
